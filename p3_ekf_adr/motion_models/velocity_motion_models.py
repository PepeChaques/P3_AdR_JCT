...
def velocity_motion_model_linearized():
    ...
    def state_transition_function_g(mu=None, u=None, delta_t=None):
        ...
        # Handle the case where omega is close to zero to avoid division by zero
        if np.abs(omega) > 1e-6:
            g = np.array([
                x + (v / omega) * (np.sin(theta + omega * delta_t) - np.sin(theta)),
                y + (v / omega) * (-np.cos(theta + omega * delta_t) + np.cos(theta)),
                theta + omega * delta_t
            ])
        else:
            # Approximation for straight-line motion
            g = np.array([
    ...
    def jacobian_of_g_wrt_state_G(mu=None, u=None, delta_t=None):
        ...
        if np.abs(omega) > 1e-6:
            G = np.array([
                [1, 0, (v / omega) * (np.cos(theta + omega * delta_t) - np.cos(theta))],
    ...
    def jacobian_of_g_wrt_control_V(mu=None, u=None, delta_t=None):
        ...
        if np.abs(omega) > 1e-6:
            V = np.array([
                [(1 / omega) * (np.sin(theta + omega * delta_t) - np.sin(theta)),
                 (v / (omega ** 2)) * (np.sin(theta) - np.sin(theta + omega * delta_t)) + (v / omega) * delta_t * np.cos(theta + omega * delta_t)],
                [(-1 / omega) * (np.cos(theta + omega * delta_t) - np.cos(theta)),
                 (v / (omega ** 2)) * (np.cos(theta + omega * delta_t) - np.cos(theta)) + (v / omega) * delta_t * np.sin(theta + omega * delta_t)],
                [0, delta_t]
            ])
        else:
            V = np.array([
